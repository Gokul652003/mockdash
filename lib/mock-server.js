...
const { matchEndpoint, matchPath } = require('./matching');
const { renderBody } = require('./body');
const { save, load } = require('./persistence');
...
class MockServer {
  ...
  _save() {
    save(this.dataFile, this.endpoints);
  }

  _load() {
    this.endpoints = load(this.dataFile, normEndpoint);
  }
  ...
  matchEndpoint(method, pathname) {
    return matchEndpoint(method, pathname, this.endpoints);
  }
  ...
  renderBody(template, ctx) {
    return renderBody(template, ctx);
  }
  ...
  handleRequest(req, res, startTime) {
    ...
    req.on('end', () => {
      let parsedBody = null;
      try { parsedBody = reqBody ? JSON.parse(reqBody) : null; } catch (_e) { parsedBody = reqBody; }
